...
import {create, getMake, getModel} from "./apiPost";
...
class NewPost extends Component {
    ...
    handleChange = (name) => event => {
        ...
        const value1 = name === 'video' ? event.target.files : event.target.value;
        if(name === 'photo'){
    ...
    clickSubmit = event => {
        ...
        if(this.isvalid()) {
            ...
            Object.values(this.state.video).forEach((file, index) => {
                // this.postData.set(`photo[${index}]`, file);
                this.postData.set(`video-${index}`, file);
            });

            this.postData.set('videos', this.state.video.length);
            ...
            create(userId, token, this.postData)
                .then(data => {
                    if (data.error) {
                        ...
                        console.log("after ", this.state.redirectToProfile);
                    }
                })
    ...
    isvalid = () =>{
        ...
        if(this.state.registration_year === 0 || this.state.registration_year === ""){
            this.setState({error: "Registeration Year required", loading :false});
            return false;
        }
        if(this.state.condition === 0 ){
            this.setState({error: "Condition is required!", loading :false})
            return false;
        }
        ...
        if(this.state.price<10000){
            this.setState({error: "Price date can not be less than 10000"});
            return false;
        }
        ...
        if( this.state.assembly === ""){
            this.setState({error: "Assembly  required", loading :false});
            return false;
        }


        if( this.state.transmission === ""){
            this.setState({error: "Transmission required", loading :false});
            return false;
        }


        if( this.state.engine_type === ""){
            this.setState({error: "Engine type required", loading :false});
            return false;
        }
    ...
    render() {
        ...
        if(this.state.redirectToProfile){
            return <Redirect to={"/postpreview/"+this.state.result._id}/>
        }
        ...
        return (
            ...
            <div>
                ...
                { isAuthenticated().user.role== 'dealer' && !isAuthenticated().user.adminVerified && (
                ...
                {  isAuthenticated().user.role != 'dealer' && isAuthenticated().user.verified && (
                    ...
                    <div className={"row"}>
                        ...
                        <div className={"col-md-6 mt-2"}>
                            ...
                            <form className={classes.form}>
                                ...
                                <InputLabel className={"mt-2"} id="Registration_city">Regitration City</InputLabel>
                                <Select
                                    labelId="Registration_city"
                                    id="Registration_city"
                                    fullWidth
                                    value={this.state.registration_city}
                                    onChange={this.handleChange("registration_city")}
                                >
                                ...
                                <InputLabel className={"mt-2"} id="color">Description</InputLabel>
                                <MDBInput
                                    type="textarea"
                                    rows="5"
                                ...
                                <div className="form-group">
                                    <label className="text-muted">Photos</label>
                                    <input
                                        required={true}
                                ...
                                <div className="form-group">
                                    <label className="text-muted">Video Upload</label>
                                    <input
                                        className="form-control"
                                        type="file"
                                        accept={".mp4,.3gp,.wmv,.mkv,.flv"}
                                        onChange={this.handleVidChange("video")}
                                    />
                                </div>
                            ...
                            assembly: "",
                            engine_capacity: "",*/}
                                        <label id="color">Engine Type</label>
                                        <Select
                                            fullWidth
                                            labelId="Engine Type"
                                            id="engine_type"
                                    ...
                                    <Grid item md={4}>
                                        <label id="color">Transmission</label>
                                        <Select
                                            fullWidth
