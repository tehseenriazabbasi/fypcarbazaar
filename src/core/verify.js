import React, {Component} from 'react';
import Button from "@material-ui/core/Button";
import { isAuthenticated, senEmail} from "../auth";




class Verify extends Component {


    clickSubmit = event => {
        event.preventDefault();
        const email = isAuthenticated().user.email;
        senEmail(email);

    };
    render() {
        return (
            <div>
                <div>
                    <div className="text-center logo-alt-box">
                        <a href="index.html" className="logo"
                           style={{"color": "#c43b3b", "font-size": "24px", "letter-spacing": ".06em", "line-height": "46px"}}>
                            <span>Carbazar.com</span>
                        </a>
                        <h5 className="text-muted m-t-0" style={{"font-size": "12px"}}>The Home of Cars</h5>
                    </div>
                    <div className="m-t-30 card-box"
                         style={{"margin-bottom": "20px", "background-clip":" padding-box", "padding": "20px", "background-color": "#ffffff", "box-shadow": "0 8px 42px 0 rgba(0, 0, 0, 0.08)"}}>
                        <div className="text-center">
                            <h4 className="text-uppercase font-bold m-b-0">Email Verification !!</h4>
                        </div>
                        <div className="panel-body text-center">
                            <img className="icon icons8-Star-Filled"
                                 src="data:image/png;base64,"
                                 width="100" height="100"/>
                            <p className="text-muted font-13 m-t-20" style={{"color": "#c43b3b"}}>
                                A email has been send to <b>your email</b>. Please check
                                for an email from company and click on the included link to verify your account
                            </p>
                            <Button
                                type="submit"
                                variant="contained"
                                onClick={this.clickSubmit}
                            >
                                Resend Email
                            </Button>
                        </div>
                    </div>
                </div>
            </div>
        );
    }
}

export default Verify;